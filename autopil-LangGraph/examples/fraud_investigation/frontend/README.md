...
A dropdown lets you pick which model runs the investigation (Gemini, free, is the
default; Claude/Anthropic is the other option) — set via `InvestigationState["provider"]`
on submit. Before the disposition is final, the run pauses for a human reviewer to
Approve or Override it via `stream.interrupt`/`stream.submit({command: {resume: ...}})`.
