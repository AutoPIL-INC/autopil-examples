...
// exactly ["defect_detection_agent"], a plain assignment, not an LLM decision (every
// quality case starts the same way: a defect gets flagged before anyone investigates
// why). Rendered distinctly from "review" (the genuinely LLM-driven re-routing among
// the 3 follow-up specialists) so the first step doesn't read as a live routing
// choice when it isn't one.
function RoutingRow({ event }: { event: FeedEvent & { type: "routing" } }) {
  const isInitial = event.stage === "initial";
  return (
    <div className={`feed-row routing ${isInitial ? "fixed-step" : ""}`}>
      <span className="feed-badge">{isInitial ? "FIXED STEP" : "ROUTE"}</span>
      <span className="feed-body">
        {isInitial
          ? `orchestrator → ${event.route?.join(", ")} (always runs first — not a routing choice)`
...
function ReviewPanel({
  ...
  // the resume payload, on BOTH approve and override — not just override. Enforce the
  // same requirement here at the form layer instead of relying on the backend's own
  // retry loop, which would otherwise silently re-pause with no visible feedback.
  const notesEmpty = notes.trim().length === 0;

  return (
    <div className="review-panel">
      <div className="review-header">Awaiting quality review — {payload.case_id}</div>
    ...
    >
      ...
      <div className="case-card-body">
        <div className="case-card-meta">
          {meta.product} · {meta.line}
          {meta.defectType ? ` · ${meta.defectType}` : ""}
        </div>
...
export default function ExecutionTab() {
  ...
  // every submit.
  const graphValues = stream.values as (InvestigationState & { __interrupt__?: Array<{ value: ReviewInterruptPayload }> }) | undefined;
        ...
        {awaitingReview && interruptPayload != null && (
          <>
            <div className="feed-row paused">
              ...
              <span className="feed-body">graph execution paused — awaiting quality review</span>
            </div>
