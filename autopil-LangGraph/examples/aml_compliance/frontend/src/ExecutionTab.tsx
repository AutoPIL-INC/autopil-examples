...
function RoutingRow({ event }: { event: FeedEvent & { type: "routing" } }) {
  return (
    <div className="feed-row routing">
      ...
      <span className="feed-body">intake → {event.route?.join(", ")}</span>
  ...
  return (
    <div className="disposition-banner">
      ...
      <div className="disposition-meta">
        {event.case_id} · roles run: {event.roles_completed.join(", ")} · denials: {event.denial_count}
        {" · "}
    ...
    >
      ...
      <div className="case-card-body">
        <div className="case-card-meta">
          Opened {formatOpenedDate(alert.openedAt)} · Priority: {alert.priority} · Account: {alert.accountId}
        </div>
        <div className="case-card-description">{alert.reasonForReview}</div>
      </div>
...
export default function ExecutionTab() {
  ...
  const stream = useStream<AMLCaseState>({
    apiUrl: API_URL,
    assistantId: "aml_compliance",
    onCustomEvent: (data) => {
  ...
  const graphValues = stream.values as (AMLCaseState & { __interrupt__?: Array<{ value: InterruptPayload }> }) | undefined;
