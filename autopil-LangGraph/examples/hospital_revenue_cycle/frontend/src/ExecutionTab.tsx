...
function DispositionBanner({ event }: { event: FeedEvent & { type: "disposition" } }) {
  return (
    <div className="disposition-banner">
      ...
      {event.revenue_recovery > 0 && (
        <div className="disposition-proposed">Revenue recovery: +${event.revenue_recovery.toLocaleString()}</div>
      )}
...
function ReviewPanel({
  ...
  return (
    <div className="review-panel">
      <div className="review-header">Awaiting revenue-cycle review — {payload.case_id}</div>
      <div className="review-proposed">Proposed: {payload.proposed_action}</div>
      {payload.revenue_recovery > 0 && (
        <div className="review-meta">
          Revenue recovery: +${payload.revenue_recovery.toLocaleString()} — {payload.action_required}
        </div>
      )}
      <div className="review-meta">
...
function CaseCard({
  ...
  const meta = ENCOUNTER_META[caseId];
  return (
    <button
    ...
    >
      ...
      <div className="case-card-body">
        <div className="case-card-meta">
          {meta.patientName} · {meta.unit} · Admitted {formatAdmissionDate(meta.admissionDate)} · Payer: {meta.payer}
        </div>
...
export default function ExecutionTab() {
  ...
  const stream = useStream<InvestigationState>({
    apiUrl: API_URL,
    assistantId: "hospital_revenue_cycle",
    onCustomEvent: (data) => {
  ...
  return (
    <div className="execution-tab">
      ...
      <div className="section-title">Encounter Queue</div>
      ...
      <div className="feed table-card">
        {feed.length === 0 && !stream.isLoading && (
          <p className="feed-empty">Pull an encounter from the queue above to start the review.</p>
        )}
        ...
        {awaitingReview && interruptPayload != null && (
          <>
            <div className="feed-row paused">
              ...
              <span className="feed-body">graph execution paused — awaiting revenue-cycle review</span>
            </div>
