...
| **Trading Desk Ops (Equities)** | 7 roles at Meridian Bank's Trading Unit handle a block equity order (a distinct ticker per scenario — MSFT, NVDA, AAPL, AMZN, GOOG) through allocation, same-day affirmation, DTCC/NSCC settlement verification, and exception investigation, inside a T+1 window. The orchestrator's trigger classification is genuinely dynamic — a new order and a PM-rebalance instruction take different graph paths — and a two-tier human review (ops-analyst vs. compliance-officer) is grounded in real inventory-shortfall/SSI-staleness data, not the case ID. First of a planned 5-sub-domain build. | [README](./autopil-LangGraph/examples/trading_desk_ops/README.md) · [DESIGN](./autopil-LangGraph/examples/trading_desk_ops/DESIGN.md) |
