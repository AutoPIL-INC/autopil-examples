# Care Coordination — Live Audit-Trail Feed

Vite + React + TypeScript viewer for the care_coordination demo one level up. Two
tabs:
...
- **Description** (`DescriptionTab.tsx`) — static reference: a visual flow diagram of
  ...
  `policies/healthcare/clinical_operations.yaml` (see `policyData.ts`), the
...
- **Execution** (`ExecutionTab.tsx`) — the live run. Streams the same events the CLI
  script prints — coordinator routing, each specialist's `[ok]`/`[DENIED]` tool calls,
  ...
  "care_coordination"`). A dropdown picks which model runs the review — Ollama
  ...
  pauses for a human care-team reviewer to Approve or Override it via
