...
Named client_analysis_saas_guard.py, not the generic saas_guard.py this file started
as — every demo with hosted-mode support had an identically-named saas_guard.py,
which collided under langgraph dev (whichever demo's graph loaded first "won," and
every other demo silently imported *that* demo's copy instead of its own). See
fraud_investigation's own fraud_saas_guard.py for how this was caught live (crashed
the whole langgraph dev server with an ImportError).

Otherwise identical to fraud_investigation's copy (kept as a per-demo copy, same
...
one client_analysis-specific finding:
  ...
  - **`wealth_advisor` also collides across demos, not just within this tenant's
    pre-seeded policies** — institutional_portfolio_review has its own `wealth_advisor`
    role too. Both demos originally shared the generic `owner_tag="autopil-langgraph-
    demos"`, and bootstrap_agents() only de-dupes by (agent_role, owner_tag) — caught
    live: institutional_portfolio_review's first bootstrap attempt silently reused
