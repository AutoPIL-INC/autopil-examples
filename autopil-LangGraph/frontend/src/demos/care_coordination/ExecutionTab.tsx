...
function RoutingRow({ event }: { event: FeedEvent & { type: "routing" } }) {
  return (
    <div className="feed-row routing">
      ...
      <span className="feed-body">
        {event.stage === "initial"
          ? `coordinator → ${event.route?.join(", ")}`
          : `coordinator review → ${event.next}`}
      </span>
...
function ReviewPanel({
  ...
  return (
    <div className="review-panel">
      <div className="review-header">Awaiting care-team review — {payload.case_id}</div>
      <div className="review-proposed">Proposed: {payload.proposed_action}</div>
      {payload.reason && <div className="review-meta">{payload.reason}</div>}
    ...
    >
      ...
      <div className="case-card-body">
        <div className="case-card-meta">
          {meta.patientName} · age {meta.age} · {meta.sex}
        </div>
    ...
    <div className="execution-tab">
      ...
      <div className="feed table-card">
        {feed.length === 0 && !stream.isLoading && (
          <p className="feed-empty">Pull a case from the queue above to start the review.</p>
        )}
        ...
        {awaitingReview && interruptPayload != null && (
          <>
            <div className="feed-row paused">
              ...
              <span className="feed-body">graph execution paused — awaiting care-team review</span>
            </div>
